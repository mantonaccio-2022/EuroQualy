...
User Function mfat02ok()
	...
	//quando a eulalia liberar a margem orçamento e o pedido de venda tirar comentario da linha de baixo
...
User Function mfat02ag()
	...
	cQuery := ChangeQuery(cQuery)
	DbUseArea(.T., "TOPCONN", TcGenQry(,,cQuery), "TRB4", .F., .F.)
		...
		If lContinua
			...
			If lContinua
				...
				If nErro <> 0
					...
					conout("cDest : " + cDest)
					conout("cDestCC : " + cDestCC)
					oMessage:cTo     := "francisco.assis@euroamerican.com.br;fabio.batista@euroamerican.com.br"
					oMessage:cCc     := "fabio@xisinformatica.com.br" //"vendas@qualyvinil.com.br"
					oMessage:cBCC    := ""
					oMessage:cSubject:= "Pedido faturado parcialmente [E-Mail Cheio ou ERRO no e-mail]"
					nErro := oMessage:Send(oMail)
					If nErro <> 0
						conout("erro send segundo: " + oMail:GetErrorString(nErro))
					EndIf
					// Fabio - Se o e-mail estiver incorreto, não deve parar o processo, enviar ao padrão
					//oMail:SMTPDisconnect()
					//TRB4->(DbCloseArea()) // FS...
					//Return .F.
				EndIf
