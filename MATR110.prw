...
#INCLUDE "PROTHEUS.CH"
#INCLUDE "REPORT.CH"
/*/{Protheus.doc} MATR11X
//Relatório utilizado para envio automático de e-mail para Representantes Comerciais no Lançamento das Comissões
//pela Equipe do Contábil. Exibido tela de confirmação da mensagem e parâmetros para usuários listados. Chamado através PE MT120FIM
...
/*/
User Function Matr11X( cAlias, nReg, nOpcx )
...
//Adicionado Euroamerican 09/03/2018 - Início
dbSelectArea("SA2")
dbSetOrder(1)
...
//MsgStop (cMAILFOR)
//MsgStop (nReg)
//Adicionado Euroamerican 09/03/2018 - Fim
...
If lTRepInUse
	...
	//Adionado Euroamerican 09/03/18 - Início
	//oReport:nDevice := 3 //nDevice --- Tipo de impressão selecionado. Opções: 1-Arquivo,2-Impressora,3-email,4-Planilha e 5-Html.
	oReport:SetDevice(3)
	oReport:CEMAIL	:= cMAILFOR
	oReport:SaveAsPDF
...
//³ExpC5 : Descricao                                                       ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("MATR11X",cTitle,"MTR110", {|oReport| ReportPrint(oReport,nReg,nOpcx)},STR0001+" "+STR0002)
oReport:SetPortrait()
...
Local nTpImp	  := IIF(ValType(oReport:nDevice)!=Nil,oReport:nDevice,1) // Tipo de Impressao  - Alterado Euroamerican 09/03/2018
