...
User Function EQFLXCX2()
    ...
    If nOpca == 1
        Pergunte(cPerg, .F.)
        Processa( {|| EQFLXCXA() }, "Extração de Dados", "Gerando Registros...", .F.)
    EndIf

Return

/*/{Protheus.doc} EQFLXCXA
...
Static Function EQFLXCXA()
    ...
    Else
        ...
        aadd(aFields, {"NOMEGR", 'C', 30, 0})
        nInicio:=5
        // Array como os nomes dos campos
        aadd(aFieldsN,"Filial")
        aadd(aFieldsN,"Grupo Natureza")
        aadd(aFieldsN,"Nome Grupo")
    End
...
@return Character, Tabelas com os dados
/*/
Static Function EQFLXREA2(lAnalitico,lConsolida,lDFC)
...
@return Character, Tabelas com os dados
/*/
Static Function  EQFLXDIA2(lAnalitico,lConsolida,lDF,cTipo)
    ...
    For nI:=0 to nDifMes
        ...
        AADD(aFields,{"_"+DTOS(CTOD(cData)),'N',14,0})
        //Nome Campo
        aadd(aFieldsN,cData)
        AADD(aDatas,cData)
    Next
...
@return Character, Tabelas com os dados
/*/
Static Function EQFLXCMP2(lAnalitico,lConsolida,lDFC)
    ...
    For nI:=0 to nDifMes
        cAnoMes:=ANOMES(MonthSum(mv_par01,nI))  // Retona o NAo e o mes seguintes a data base
        AADD(aFields,{"_"+cAnoMes+"P",'N',14,0})
        AADD(aFields,{"_"+cAnoMes+"R",'N',14,0})
...
@return Character, Tabelas com os dados
/*/
Static Function  EQFLXEXC2(lAnalitico,lConsolida,lDFC)
    ...
    //Indica quais campos entrrao na montaem do EXCEL
    cCampos:="%"
    ...
    If Mv_par05 == 1
        cTitPla+="Previst0 "
        cArqDst+="PRV_"+ Dtos(dDataBase) + StrTran(Time(),":","") + ".XML"
    ElseIf Mv_par05 == 2
        cTitPla+="Realizado"
    ...
    ElseIf Mv_par05 == 5
        ...
        cArqDst+="DIA_REA_"+ Dtos(dDataBase) + StrTran(Time(),":","") + ".XML"
    End
    ...
    For nJ:=1 To Len(aFilial)
        ...
        For nI := 1 To nCampos
            ...
            ElseIf aFields[nI+1,2] == "N"
                oExcel:AddColumn(cNomeFil, cTitPla, aFieldsN[nI+1], 3, 2, .F.)
            End
        Next
    ...
    oExcel:AddTable ("Parametros","Definições")

    oExcel:AddColumn("Parametros","Definições","Ordem",1,1)
    oExcel:AddColumn("Parametros","Definições","Pergunta",1,1)
    oExcel:AddColumn("Parametros","Definições","Resposta",1,1)
    ...
    For nI:=1 to Len(aPergunte[2])
        ...
        End
        oExcel:AddRow("Parametros","Definições",{aPergunte[2][nI]:CX1_ORDEM,RTRIM(aPergunte[2][nI]:CX1_PERGUNT),cVar})
    Next
...
/*/
Static Function EQFLXST2(lAnalitico,lConsolida,lDFC)
    ...
    cDia:=CTOD("01/"+StrZero(Month(mv_par01),2,0)+"/"+Str(Year(mv_par01),4,0)) - 1

    BeginSql Alias _SE8
        SELECT
            SUBSTRING (SE8.E8_FILIAL, 1, 2) AS FILIAL,
            SUM (SE8.E8_SALATUA) AS VALOR
        FROM
            %Table:SE8% SE8
        INNER JOIN %Table:SA6% SA6
        ON SUBSTRING (SE8.E8_FILIAL, 1, 2) = SA6.A6_FILIAL
            AND SE8.E8_BANCO = SA6.A6_COD
            AND SA6.A6_FLUXCAI = 'S'
            AND SA6.D_E_L_E_T_ = ' '
        WHERE
            SE8.D_E_L_E_T_ = ' '
            AND SE8.E8_DTSALAT = %Exp:cDia%
        GROUP BY
...
Montas as querys de filtragem para o fluxo conforme o tipo
@type function Processamento
@version  1.00
@author mario.antonaccio
@since 08/04/2022
@param Tipo, character, Qual o Fluxo que será gerado
@param lFilial, logical, indica se separa ou nao por filial -  para os casos de consolidado
@return Numeric, Numero Total de registros
/*/

Static Function _FQUERY(TIPO,LFILIAL)
    ...
    cCpoQry   := "% "+If(lFilial,", FILIAL","")+" NATUREZA, MESANO, SUM(VALOR) AS VALOR "+"%"
    
    cGrpBy    := "% "+If(lFilial," FILIAL,","")+" NATUREZA, MESANO"+"%"

    If TIPO == "PRV"  //Fluxo PRevisto

        cCpoSQry1:="%"+If(lFilial,"SUBSTRING(SE1.E1_FILIAL, 1, 2) AS FILIAL,","")+"%"
        cCpoSQry2:="%"+If(lFilial,"SUBSTRING(SE2.E2_FILIAL, 1, 2) AS FILIAL,","")+"%"

        cWhere1:="%"+If(lFilial," AND SUBSTRING(SE1.E1_FILIAL, 1, 2) IN %Exp:cExprEmp%","")+"%"
        cWhere2:="%"+If(lFilial," AND SUBSTRING(SE2.E2_FILIAL, 1, 2) IN %Exp:cExprEmp%","")+"%"
