...
Static Function MenuDef()
    ...
    //ADD OPTION aMenu TITLE 'Incluir' 	ACTION 'VIEWDEF.IncZ02'	OPERATION 3 ACCESS 0
...
Static Function EQCALCM6()
    ...
    oExcel:AddColumn(cNomPla, cTitPla, "Pontuacao de"   , 1, 1, .F.)
    oExcel:AddColumn(cNomPla, cTitPla, "PontuaÁ„o Ate"  , 1, 4, .F.)
    ...
    oExcel:AddTable ("Parametros","DefiniÁıes", cTitPla+" (EQCALCOM)")
    oExcel:AddColumn("Parametros","DefiniÁıes","Ordem",1,1)
    oExcel:AddColumn("Parametros","DefiniÁıes","Pergunta",1,1)
    oExcel:AddColumn("Parametros","DefiniÁıes","Resposta",1,1)
    ...
    For nI:=1 to Len(aPergunte[2])
        ...
        End
        oExcel:AddRow("Parametros","DefiniÁıes",{aPergunte[2][nI]:CX1_ORDEM,RTRIM(aPergunte[2][nI]:CX1_PERGUNT),cVar})
    Next
    //Fim Parametros
    (_FZ02)->(dbCloseArea())
