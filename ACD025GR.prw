...
User Function ACD025GR() //	ExecBlock("ACD025GR",.F.,.F.,{cOp,cOperacao,cRecurso,cOperador,nQtd,cTransac})
...
Local lEnvase     := AllTrim( cOperacao ) == "50"
Local lEnvia      := .T.
Local cQuery      := ""
Local nEnvase     := 0
Local nOPEnv      := 0
...
If lEnvia
	...
	If AllTrim( cOperacao ) == "10" .And. AllTrim( cTransac ) == "04"
		dbSelectArea("SC2")
		dbSetOrder(1)
	...
	ElseIf AllTrim( cOperacao ) == "20" .And. AllTrim( cTransac ) == "04"
		dbSelectArea("SC2")
		dbSetOrder(1)
		If SC2->( dbSeek( xFilial("SC2") + cOP ) )
			dbSelectArea("SB1")
