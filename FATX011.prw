...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
Do Case
		Case ( nOpcUsr >= 03 .And. nOpcUsr <= 08 ) .And. .Not. lUsrOpc
...
If nOpc == 10 .Or. nOpc == 11 .Or. nOpc == 13
    ...
    If !INCLUI
	    ...
	    If !Empty( cCliFor ) .And. !Empty( cLoja )
			...
			cQry += "			END [CODMUN] " + ENTER
			cQry += "		FROM  " + ENTER
			cQry += "			SF2" + Subs(cIDEmp,1,2) + "0 SF2  " + ENTER
...
If nOpc == 15
	...
	If ( nOpcUsr == 9 .Or. nOpcUsr == 10 ) .And. M->ZF_TIPO == "1"
		...
		cQry += "FROM	SD1" + Left( aCols[nAT][nPEmpFil], 2 ) + "0 " + ENTER
		cQry += "WHERE	D_E_L_E_T_ = '' " + ENTER
...
Local lUserDIR   := Upper(AllTrim(cUserName)) $ "ADMINISTRADOR#CAROLINE.MONEA#THIAGO.MONEA#ROBSON.MORAES#ANTONIO.BARBOSA#CARLOS.ARAUJO"		//Retirado Alexandre.Martins em 23/03/18 U_IsGroup("Administradores#DIR-X00")
...
If nFrete > 5
	...
	If At("IT06", SZF->ZF_LOG) == 0
		...
		Else
			...
			//ณ Chama rotina padrao do programa LIBSMS                              ณ
			//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
			U_SMSMarkBrw( "002", IIf( nFrete > 7, 5, 1 ) )
