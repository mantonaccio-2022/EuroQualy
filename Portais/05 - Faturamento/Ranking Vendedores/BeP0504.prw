...
Local lGerente  := .F.
Local cCodGer   := ""
Local cDiretor  := "000303|000370|000736|001028|000867|000995|000000"
...
/********************************************************************************************************************************/
	...
	cQuery += "AND SC6.D_E_L_E_T_ = ' ' " + CRLF
	cQuery += "UNION ALL " + CRLF
	cQuery += "SELECT 'Jays' AS EMPRESA, 'Faturado' AS TIPO, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET), 0) AS VALOR, ISNULL(SUM(D2_CUSTO1), 0) AS CUSTOMEDIO, ISNULL(SUM(D2_QUANT * B1_CUSTD),0) AS CUSTOSTANDARD " + CRLF
	cQuery += "FROM " + RetSqlName("SD2") + " AS SD2 WITH (NOLOCK) " + CRLF
	cQuery += "INNER JOIN " + RetSqlName("SF2") + " AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
	cQuery += "  AND F2_DOC = D2_DOC " + CRLF
	cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
	...
	cQuery += "AND SD2.D_E_L_E_T_ = ' ' " + CRLF
	cQuery += "UNION ALL " + CRLF
	cQuery += "SELECT 'Jays' AS EMPRESA, 'Devolvido' AS TIPO, ISNULL(SUM((D1_TOTAL - D1_VALDESC + SD1.D1_VALIPI + D1_DESPESA + D1_VALFRE + D1_SEGURO + D1_ICMSRET)) * (-1), 0) AS VALOR, ISNULL(SUM(D1_CUSTO) * (-1), 0) AS CUSTOMEDIO, ISNULL(SUM(D1_QUANT * B1_CUSTD) * (-1),0) AS CUSTOSTANDARD " + CRLF
	cQuery += "FROM " + RetSqlName("SD1") + " AS SD1 WITH (NOLOCK) " + CRLF
	cQuery += "INNER JOIN " + RetSqlName("SD2") + " AS SD2 WITH (NOLOCK) ON D2_FILIAL = D1_FILIAL " + CRLF
	cQuery += "  AND D2_DOC = D1_NFORI " + CRLF
	cQuery += "  AND D2_SERIE = D1_SERIORI " + CRLF
	...
	cQuery += "AND SD1.D_E_L_E_T_ = ' ' " + CRLF
	cQuery += "UNION ALL " + CRLF
	cQuery += "SELECT 'Jays' AS EMPRESA, 'Carteira' AS TIPO, ISNULL(SUM(((C6_QTDVEN - C6_QTDENT ) * C6_PRCVEN)), 0) AS VALOR, ISNULL(SUM(((C6_QTDVEN - C6_QTDENT ) * B2_CM1)), 0) AS CUSTOMEDIO, ISNULL(SUM((C6_QTDVEN - C6_QTDENT ) * B1_CUSTD),0) AS CUSTOSTANDARD " + CRLF
	cQuery += "FROM " + RetSqlName("SC6") + " AS SC6 WITH (NOLOCK) " + CRLF
	cQuery += "INNER JOIN " + RetSqlName("SC5") + " AS SC5 WITH (NOLOCK) ON C5_FILIAL = C6_FILIAL " + CRLF
	cQuery += "  AND C5_NUM = C6_NUM " + CRLF
	cQuery += "  AND C5_TIPO = 'N' " + CRLF
...
&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	If lDiretor
		...
		cQuery += "SELECT '92' AS FILIAL, 'Metropole' AS EMPRESA, CASE WHEN D2_SEGUM = 'L' AND B1_CONV <> 0 THEN D2_QTSEGUM ELSE D2_QUANT END AS VOLUME, CASE WHEN B1_SEGUM = 'L' AND B1_CONV <> 0 THEN CASE WHEN B1_TIPCONV = 'M' THEN D2_PRCVEN / B1_CONV ELSE D2_PRCVEN * B1_CONV END ELSE D2_PRCVEN END AS PRECO " + CRLF
		cQuery += "FROM " + RetSqlName("SD2") + " AS SD2 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN " + RetSqlName("SF2") + " AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
		cQuery += "  AND F2_DOC = D2_DOC " + CRLF
		cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
		...
		cQuery += "SELECT '93' AS FILIAL, 'Qualyvinil' AS EMPRESA, CASE WHEN D2_SEGUM = 'L' AND B1_CONV <> 0 THEN D2_QTSEGUM ELSE D2_QUANT END AS VOLUME, CASE WHEN B1_SEGUM = 'L' AND B1_CONV <> 0 THEN CASE WHEN B1_TIPCONV = 'M' THEN D2_PRCVEN / B1_CONV ELSE D2_PRCVEN * B1_CONV END ELSE D2_PRCVEN END AS PRECO " + CRLF
		cQuery += "FROM " + RetSqlName("SD2") + " AS SD2 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN " + RetSqlName("SF2") + " AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
		cQuery += "  AND F2_DOC = D2_DOC " + CRLF
		cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
		...
		*/
		...
		//cHtml += '      <table id="example" class="table table-striped table-bordered table-condensed col-md-12" style="width:100%">'+CRLF
		cHtml += '    		<thead>'+CRLF
		cHtml += '      		<tr>'+CRLF
...
&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	If (lFilUsr .And. AllTrim( HttpSession->ccodusr ) $ cSoEuro) .Or. !lFilUsr
		//cQuery := "SELECT GERENTE, VENDEDOR, NOME, SUM(TOTAL) AS TOTAL, SUM(DEVOLUCAO) AS DEVOLUCAO, SUM(CARTEIRA) AS CARTEIRA, SUM(CREDITO) AS CREDITO, SUM(ESTOQUE) AS ESTOQUE, SUM(AFATURAR) AS AFATURAR, SUM(BRUTO) AS BRUTO, SUM(META) AS META, SUM(BASECOM) AS BASECOM, SUM(VENDDIA) AS VENDDIA, SUM(CUSTO) AS CUSTO, SUM(VALOR) AS VALOR " + CRLF
	...
	/*************************************************************************************************************************************/
		cQuery += "UNION ALL " + CRLF
		//cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, ISNULL(SUM(((C6_QTDVEN - C6_QTDENT ) * C6_PRCVEN)), 0) AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
	...
	/*************************************************************************************************************************************/
		...
		//Table
		//cHtml += '	<div id="example" class="col-md-12" style="overflow-x:auto; width=100%; overflow-y:auto;">'+CRLF
		...
		//cHtml += '        			<th>Pedido de Vendas</th>'+CRLF
		...
		Do While !TMP001->( Eof() )
			If AllTrim( cGerente ) <> AllTrim( TMP001->GERENTE )
				If AllTrim( cGerente ) <> "&%*$@#"
					cHtml += '				<tr>'+CRLF
					cHtml += '        			<td></td>'+CRLF
					//cHtml += '        			<td colspan="2"><B><I><font color="DimGray">Sub-Total Gerente:</font></I></B></td>'+CRLF
					cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
					cHtml += '        			<td><B><I><font color="DimGray">Sub-Total Gerente:</font></I></B></td>'+CRLF
					//cHtml += '        			<td style="text-align: right;"><B><I><font color="DimGray">'+Transform(nGerBru,PesqPict("SB2","B2_VATU1"))+'</font></I></B></td>'+CRLF
					cHtml += '        			<td style="text-align: right;"><B><I><font color="DimGray">'+Transform(nGerMet,PesqPict("SB2","B2_VATU1"))+'</font></I></B></td>'+CRLF
					//cHtml += '        			<td style="text-align: right;"><B><I><font color="DimGray">'+Transform(nGerTot,PesqPict("SB2","B2_VATU1"))+'</font></I></B></td>'+CRLF
					...
					//cHtml += '        			<td style="text-align: right;"><B><I><font color="DimGray">'+Transform(nGerCar + nGerCre + nGerEst + nGerFat,PesqPict("SB2","B2_VATU1"))+'</font></I></B></td>'+CRLF
				...
				If Empty(cGerente)
					If lDiretor
						//cHtml += '        			<td colspan="14"><B><I><font color="DimGray">[Não Possui Gerente]</font></I></B></td>'+CRLF
						...
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
					Else
						//cHtml += '        			<td colspan="13"><B><I><font color="DimGray">[Não Possui Gerente]</font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray">[Não Possui Gerente]</font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
						//cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
				...
				Else
					If lDiretor
						//cHtml += '        			<td colspan="14"><B><I><font color="DimGray">'+Posicione("SA3", 1, xFilial("SA3") + cGerente, "A3_NOME")+'</font></I></B></td>'+CRLF
						...
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
					Else
						//cHtml += '        			<td colspan="13"><B><I><font color="DimGray">'+Posicione("SA3", 1, xFilial("SA3") + cGerente, "A3_NOME")+'</font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray">'+Posicione("SA3", 1, xFilial("SA3") + cGerente, "A3_NOME")+'</font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
						//cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
						cHtml += '        			<td><B><I><font color="DimGray"></font></I></B></td>'+CRLF
			...
			nPrcMed   := nEurMed
			//nPrcMed   += TMP001->PRECOMEDIO
			//nGerMed   += TMP001->PRECOMEDIO
		...
		//cHtml += '        			<td style="text-align: right;"><B>'+Transform(nCarteira + nCredito + nEstoque + nAFaturar,PesqPict("SB2","B2_VATU1"))+'</B></td>'+CRLF
...
/*
		...
		cQuery += "FROM  ( " + CRLF
		//cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET) AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, SUM(D2_VALBRUT) AS BRUTO, 0 AS META, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_ICMSRET), 0) AS BASECOM, 0 AS VENDDIA, SUM(D2_CUSTO1) CUSTO, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET), 0) AS VALOR " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET) AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, SUM(D2_VALBRUT) AS BRUTO, 0 AS META, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_ICMSRET), 0) AS BASECOM, 0 AS VENDDIA, SUM(D2_CUSTO1) CUSTO, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET), 0) AS VALOR, SUM(CASE WHEN D2_SEGUM = 'L' AND B1_CONV <> 0 THEN D2_QTSEGUM ELSE D2_QUANT END) AS VOLUME, AVG(CASE WHEN B1_SEGUM = 'L' AND B1_CONV <> 0 THEN CASE WHEN B1_TIPCONV = 'M' THEN D2_PRCVEN / B1_CONV ELSE D2_PRCVEN * B1_CONV END ELSE D2_PRCVEN END) AS PRECOMEDIO " + CRLF
		cQuery += "FROM SD2080 AS SD2 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SF2080 AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
		cQuery += "  AND F2_DOC = D2_DOC " + CRLF
		cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
		cQuery += "  AND F2_CLIENTE = D2_CLIENTE " + CRLF
		cQuery += "  AND F2_LOJA = D2_LOJA " + CRLF
		...
		EndIf
		cQuery += "  AND SF2.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SF4080 AS SF4 WITH (NOLOCK) ON F4_FILIAL = '  ' " + CRLF
		cQuery += "  AND F4_CODIGO = D2_TES " + CRLF
		cQuery += "  AND F4_DUPLIC = 'S' " + CRLF
		cQuery += "  AND SF4.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SB1000 AS SB1 WITH (NOLOCK) ON B1_FILIAL = '  ' " + CRLF
		cQuery += "  AND B1_COD = D2_COD " + CRLF
		cQuery += "  AND SB1.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "LEFT JOIN SA3000 AS SA3 WITH (NOLOCK) ON A3_FILIAL = '  ' " + CRLF
		cQuery += "  AND A3_COD = F2_VEND1 " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE D2_FILIAL = '03' " + CRLF
		cQuery += "AND D2_TIPO = 'N' " + CRLF
		cQuery += "AND LEFT(D2_EMISSAO, 6) = '" + Left( DTOS( dDataBase ), 6) + "' " + CRLF
		...
		cQuery += "INNER JOIN SD2080 AS SD2 WITH (NOLOCK) ON D2_FILIAL = D1_FILIAL " + CRLF
		cQuery += "  AND D2_DOC = D1_NFORI " + CRLF
		cQuery += "  AND D2_SERIE = D1_SERIORI " + CRLF
		cQuery += "  AND D2_CLIENTE = D1_FORNECE " + CRLF
		cQuery += "  AND D2_LOJA = D1_LOJA " + CRLF
		...
		cQuery += "  AND A3_COD = F2_VEND1 " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE D1_FILIAL = '03' " + CRLF
		cQuery += "AND D1_TIPO = 'D' " + CRLF
		cQuery += "AND LEFT(D1_DTDIGIT, 6) = '" + Left( DTOS( dDataBase ), 6) + "' " + CRLF
		cQuery += "AND SD1.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "GROUP BY A3_GEREN, A3_COD, A3_NOME " + CRLF
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, ISNULL(SUM(((C6_QTDVEN - C6_QTDENT ) * C6_PRCVEN)), 0) AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SC6080 AS SC6 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SC5080 AS SC5 WITH (NOLOCK) ON C5_FILIAL = C6_FILIAL " + CRLF
		cQuery += "  AND C5_NUM = C6_NUM " + CRLF
		cQuery += "  AND C5_TIPO = 'N' " + CRLF
		cQuery += "  AND C5_LIBEROK = '' " + CRLF
		cQuery += "  AND NOT EXISTS (SELECT A1_FILIAL FROM SA1080 WITH (NOLOCK) WHERE A1_FILIAL = C5_FILIAL AND A1_COD = C5_CLIENTE AND A1_LOJA = C5_LOJACLI AND A1_EQ_PRRL = '1' AND D_E_L_E_T_ = ' ') " + CRLF
		If lGerente
		...
		cQuery += "  AND B1_COD = C6_PRODUTO " + CRLF
		cQuery += "  AND SB1.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SB2080 AS SB2 WITH (NOLOCK) ON B2_FILIAL = C6_FILIAL " + CRLF
		cQuery += "  AND B2_COD = C6_PRODUTO " + CRLF
		cQuery += "  AND B2_LOCAL = C6_LOCAL " + CRLF
		cQuery += "  AND SB2.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "LEFT JOIN SA3000 AS SA3 WITH (NOLOCK) ON A3_FILIAL = '  ' " + CRLF
		cQuery += "  AND A3_COD = C5_VEND1 " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE C6_FILIAL = '03' " + CRLF
		cQuery += "AND C6_BLQ = '' " + CRLF
		cQuery += "AND C6_QTDVEN > C6_QTDENT " + CRLF
		cQuery += "AND SC6.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "GROUP BY A3_GEREN, A3_COD, A3_NOME " + CRLF
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, SUM((C9_QTDLIB * C9_PRCVEN)) AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SC9080 AS SC9 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SC6080 AS SC6 WITH (NOLOCK) ON C6_FILIAL = C9_FILIAL " + CRLF
		cQuery += "  AND C6_NUM = C9_PEDIDO " + CRLF
		cQuery += "  AND C6_ITEM = C9_ITEM " + CRLF
		cQuery += "  AND C6_BLQ = '' " + CRLF
		cQuery += "  AND C6_QTDVEN > C6_QTDENT " + CRLF
		...
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, SUM(CT_VALOR) AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SCT080 AS SCT WITH (NOLOCK) " + CRLF
		cQuery += "LEFT JOIN SA3000 AS SA3 WITH (NOLOCK) ON A3_FILIAL = '  ' " + CRLF
		cQuery += "  AND A3_COD = CT_VEND " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE CT_FILIAL = '  ' " + CRLF
		If lGerente
			cQuery += "  AND EXISTS (SELECT A3_FILIAL FROM SA3000 AS SA3 WITH (NOLOCK) WHERE A3_FILIAL = '  ' AND A3_COD = CT_VEND AND A3_GEREN = '" + AllTrim( cCodGer ) + "' AND SA3.D_E_L_E_T_ = ' ') " + CRLF
		EndIf
		cQuery += "AND LEFT(CT_DATA, 6) = '" + Left( DTOS( dDataBase ), 6) + "' " + CRLF
		cQuery += "AND CT_MSBLQL <> '1' " + CRLF
		cQuery += "AND SCT.D_E_L_E_T_ = ' ' " + CRLF
...
/*
		...
		//cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET) AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, SUM(D2_VALBRUT) AS BRUTO, 0 AS META, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_ICMSRET), 0) AS BASECOM, 0 AS VENDDIA, SUM(D2_CUSTO1) CUSTO, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET), 0) AS VALOR " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET) AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, SUM(D2_VALBRUT) AS BRUTO, 0 AS META, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_ICMSRET), 0) AS BASECOM, 0 AS VENDDIA, SUM(D2_CUSTO1) CUSTO, ISNULL(SUM(D2_VALBRUT - D2_VALIPI - D2_VALICM - D2_VALIMP5 - D2_VALIMP6 - D2_DESPESA - D2_VALFRE - D2_DESCON - D2_SEGURO - D2_ICMSRET), 0) AS VALOR, SUM(CASE WHEN D2_SEGUM = 'L' AND B1_CONV <> 0 THEN D2_QTSEGUM ELSE D2_QUANT END) AS VOLUME, AVG(CASE WHEN B1_SEGUM = 'L' AND B1_CONV <> 0 THEN CASE WHEN B1_TIPCONV = 'M' THEN D2_PRCVEN / B1_CONV ELSE D2_PRCVEN * B1_CONV END ELSE D2_PRCVEN END) AS PRECOMEDIO " + CRLF
		cQuery += "FROM SD2010 AS SD2 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SF2010 AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
		cQuery += "  AND F2_DOC = D2_DOC " + CRLF
		cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
		cQuery += "  AND F2_CLIENTE = D2_CLIENTE " + CRLF
		cQuery += "  AND F2_LOJA = D2_LOJA " + CRLF
		...
		EndIf
		cQuery += "  AND SF2.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SF4010 AS SF4 WITH (NOLOCK) ON F4_FILIAL = '  ' " + CRLF
		cQuery += "  AND F4_CODIGO = D2_TES " + CRLF
		cQuery += "  AND F4_DUPLIC = 'S' " + CRLF
		cQuery += "  AND SF4.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SB1000 AS SB1 WITH (NOLOCK) ON B1_FILIAL = '  ' " + CRLF
		...
		cQuery += "AND D2_TIPO = 'N' " + CRLF
		cQuery += "AND LEFT(D2_EMISSAO, 6) = '" + Left( DTOS( dDataBase ), 6) + "' " + CRLF
		cQuery += "AND NOT EXISTS (SELECT A1_FILIAL FROM SA1010 WITH (NOLOCK) WHERE A1_FILIAL = D2_FILIAL AND A1_COD = D2_CLIENTE AND A1_LOJA = D2_LOJA AND A1_EQ_PRRL = '1' AND D_E_L_E_T_ = ' ') " + CRLF
		cQuery += "AND SD2.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "GROUP BY A3_GEREN, A3_COD, A3_NOME " + CRLF
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, SUM((D1_TOTAL-D1_VALDESC-D1_VALIPI+D1_DESPESA+D1_VALFRE+D1_SEGURO-D1_ICMSRET-D1_VALICM-D1_VALIMP5-D1_VALIMP6)) * (-1) AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SD1010 AS SD1 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SD2010 AS SD2 WITH (NOLOCK) ON D2_FILIAL = D1_FILIAL " + CRLF
		cQuery += "  AND D2_DOC = D1_NFORI " + CRLF
		cQuery += "  AND D2_SERIE = D1_SERIORI " + CRLF
		cQuery += "  AND D2_CLIENTE = D1_FORNECE " + CRLF
		cQuery += "  AND D2_LOJA = D1_LOJA " + CRLF
		...
		cQuery += "  AND A3_COD = F2_VEND1 " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE D1_FILIAL = '07' " + CRLF
		cQuery += "AND D1_TIPO = 'D' " + CRLF
		cQuery += "AND LEFT(D1_DTDIGIT, 6) = '" + Left( DTOS( dDataBase ), 6) + "' " + CRLF
		cQuery += "AND SD1.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "GROUP BY A3_GEREN, A3_COD, A3_NOME " + CRLF
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, ISNULL(SUM(((C6_QTDVEN - C6_QTDENT ) * C6_PRCVEN)), 0) AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SC6010 AS SC6 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SC5010 AS SC5 WITH (NOLOCK) ON C5_FILIAL = C6_FILIAL " + CRLF
		cQuery += "  AND C5_NUM = C6_NUM " + CRLF
		cQuery += "  AND C5_TIPO = 'N' " + CRLF
		cQuery += "  AND C5_LIBEROK = '' " + CRLF
		cQuery += "  AND NOT EXISTS (SELECT A1_FILIAL FROM SA1010 WITH (NOLOCK) WHERE A1_FILIAL = C5_FILIAL AND A1_COD = C5_CLIENTE AND A1_LOJA = C5_LOJACLI AND A1_EQ_PRRL = '1' AND D_E_L_E_T_ = ' ') " + CRLF
		If lGerente
			cQuery += "  AND EXISTS (SELECT A3_FILIAL FROM SA3000 AS SA3 WITH (NOLOCK) WHERE A3_FILIAL = '  ' AND A3_COD = C5_VEND1 AND A3_GEREN = '" + AllTrim( cCodGer ) + "' AND SA3.D_E_L_E_T_ = ' ') " + CRLF
		EndIf
		cQuery += "  AND SC5.D_E_L_E_T_ = ' ' " + CRLF
		...
		cQuery += "  AND B1_COD = C6_PRODUTO " + CRLF
		cQuery += "  AND SB1.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN SB2010 AS SB2 WITH (NOLOCK) ON B2_FILIAL = C6_FILIAL " + CRLF
		cQuery += "  AND B2_COD = C6_PRODUTO " + CRLF
		cQuery += "  AND B2_LOCAL = C6_LOCAL " + CRLF
		cQuery += "  AND SB2.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "LEFT JOIN SA3000 AS SA3 WITH (NOLOCK) ON A3_FILIAL = '  ' " + CRLF
		cQuery += "  AND A3_COD = C5_VEND1 " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE C6_FILIAL = '07' " + CRLF
		cQuery += "AND C6_BLQ = '' " + CRLF
		cQuery += "AND C6_QTDVEN > C6_QTDENT " + CRLF
		cQuery += "AND SC6.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "GROUP BY A3_GEREN, A3_COD, A3_NOME " + CRLF
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, SUM((C9_QTDLIB * C9_PRCVEN)) AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, 0 AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SC9010 AS SC9 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN SC6010 AS SC6 WITH (NOLOCK) ON C6_FILIAL = C9_FILIAL " + CRLF
		cQuery += "  AND C6_NUM = C9_PEDIDO " + CRLF
		cQuery += "  AND C6_ITEM = C9_ITEM " + CRLF
		cQuery += "  AND C6_BLQ = '' " + CRLF
		cQuery += "  AND C6_QTDVEN > C6_QTDENT " + CRLF
		...
		cQuery += "UNION ALL " + CRLF
		cQuery += "SELECT ISNULL(A3_GEREN, '') AS GERENTE, ISNULL(A3_COD, '') AS VENDEDOR, ISNULL(A3_NOME, '') AS NOME, 0 AS TOTAL, 0 AS DEVOLUCAO, 0 AS CARTEIRA, 0 AS CREDITO, 0 AS ESTOQUE, 0 AS AFATURAR, 0 AS BRUTO, SUM(CT_VALOR) AS META, 0 AS BASECOM, 0 AS VENDDIA, 0 AS CUSTO, 0 AS VALOR, 0 AS VOLUME, 0 AS PRECOMEDIO " + CRLF
		cQuery += "FROM SCT010 AS SCT WITH (NOLOCK) " + CRLF
		cQuery += "LEFT JOIN SA3000 AS SA3 WITH (NOLOCK) ON A3_FILIAL = '  ' " + CRLF
		cQuery += "  AND A3_COD = CT_VEND " + CRLF
		cQuery += "  AND SA3.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "WHERE CT_FILIAL = '  ' " + CRLF
