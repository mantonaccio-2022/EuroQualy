...
Local cAlias	 := ""
//Local cFilFil    := "'  ','00','01','02','03','04','05','06','07','08'"
...
If lSession
	...
	If lApvComp
		...
		cQuery += "INNER JOIN " + RetSqlName("SAL") + " AS SAL WITH (NOLOCK) ON AL_FILIAL = '" + xFilial("SAL") + "' " + CRLF
		cQuery += "  AND AL_COD = '" + GetMv("MV_NFAPROV",,"000002") + "' " + CRLF
		cQuery += "  AND AL_APROV = CR_APROV " + CRLF
		cQuery += "  AND AL_USER = CR_USER " + CRLF
		cQuery += "  AND SAL.D_E_L_E_T_ = ' ' " + CRLF
		cQuery += "INNER JOIN " + RetSqlName("SAK") + " AS SAK WITH (NOLOCK) ON AK_FILIAL = '" + xFilial("SAK") + "' " + CRLF
...
&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
		...
		/*
		cHtml += '			<div class="col-lg-6 col-md-6">' + CRLF
		cHtml += '				<h5><i class="fa fa-bar-chart-o fa-fw"></i> Por Filial no Ano (<B>' + Left( DTOS( dDataBase ), 4) + '</B>)</h5>' + CRLF
		cHtml += '				<hr/>' + CRLF
		cHtml += '					<div class="well well-lg">' + CRLF
		cHtml += '					<canvas id="doughnutChart"></canvas>' + CRLF
		cHtml += '					<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.1/Chart.min.js"></script>' + CRLF
		cHtml += '					<script>' + CRLF
		cHtml += "						var ctxD = document.getElementById(" + '"doughnutChart"' + ").getContext('2d');" + CRLF
		cHtml += '						var myLineChart = new Chart(ctxD, {' + CRLF
		cHtml += "						type: 'doughnut'," + CRLF
		cHtml += '						data: {' + CRLF
		cHtml += '						labels: ["01 - Cosmotec Guarulhos", "02 - Cosmotec São Paulo"],' + CRLF
		cHtml += '						datasets: [' + CRLF
		cHtml += '						{' + CRLF
		cHtml += '						data: [' + AllTrim( Str(nFil01) ) + ', ' + AllTrim( Str(nFil02) ) + '],' + CRLF
		cHtml += '						backgroundColor: ["#F7464A", "#949FB1"],' + CRLF
		cHtml += '						hoverBackgroundColor: ["#FF5A5E", "#A8B3C5"]' + CRLF
		cHtml += '						}' + CRLF
		cHtml += '						]' + CRLF
		cHtml += '						},' + CRLF
		cHtml += '						options: {' + CRLF
		...
		*/
		...
		cHtml += '			</div>' + CRLF
		/*
		If !lApvCart .And. !lColabor
			cHtml += '		</div>' + CRLF
		    cHtml += '		<script src="data/flot-data.js"></script>' + CRLF
		    cHtml += '		<FilesMatch "\.(ttf|ttc|otf|eot|woff)$">' + CRLF
		    cHtml += '			<IfModule mod_headers.c>' + CRLF
		    cHtml += '			</IfModule>' + CRLF
		    cHtml += '		</FilesMatch>' + CRLF
			cHtml += '	</div>' + CRLF
		EndIf
		*/
    EndIf
