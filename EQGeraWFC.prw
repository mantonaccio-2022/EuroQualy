...
±±ºPrograma  ³ EQGERAWFC ºAutor  ³Microsiga          º Data ³  08/31/20   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Rotina padrão para geração de WF do controle               º±±
±±º          ³ de recebimentos/compras                                    º±±
...
User Function EQGeraWFC( _cAssunto, _cStatus, _cTxtSts, _cHTML, _cDescrFase )
	...
	If lHTML
		...
		// Processo de cotação - NOVO
		If Len( aWorkflow ) == 0

			SC8->(dbSetOrder(1))
			If SC8->(dbSeek(xFilial("SC8")+SC7->C7_NUMCOT+SC7->C7_FORNECE+SC7->C7_LOJA))
				aMotVenc:=zMemoToA(SC8->C8_MOTVENC, 80, cQuebra, lTiraBra)
				For nI:=1 to Len(aMotVenc)
					cMotVenc+=RTRIM(aMotvenc[nI])+CRLF
				End
				iF !Empty(cMotvenc)
					aAdd( (oHtml:valbyname( "itWF.dWFData" 		)), DTOC(SC8->C8_EMISSAO)							)
					aAdd( (oHtml:valbyname( "itWF.cWFHora"		)), ""												)
					aAdd( (oHtml:valbyname( "itWF.cWFUsuario"	)), "COTACAO-MOT VENCEDOR"									    )
					aAdd( (oHtml:valbyname( "itWF.cWFProc"		)), cMotVenc												)
					aAdd( (oHtml:valbyname( "itWF.cWFTempo"		)), ""												)
				End
			End
		EndIf
