...
/*/
User Function BOLRENDIM(nNum,serie,cliente,loja,nopc)
...
Static Function MontaRel()
	...
	Private _nVlrAbat  := 0
	Private cAgRend    := MV_PAR20
	Private cCntRend  := mv_par21
	Private cDVRend    :=mv_par22
	...
	oPrint:=TMSPrinter():New( "Emissão de Boletos [Banco Rendimentos]" )
	oPrint:SetPortrait()
	oPrint:StartPage()
	...
	For it:= 1 To Len( aVetor )
		...
		aDadosBanco  := {"633",;			// [1]Numero do Banco
			"Banco Rendimentos ",;// [2]Nome do Banco
			cAgRend,;			// [3]Agência
			cCntRend,;			// [4]Conta Corrente
			cDVRend,;			// [5]Dígito da conta corrente
...
Static Function Impress(oPrint,aBitmap,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,CB_RN_NN)
	...
	aBmp2 := "\SYSTEM\Rendimentos_Bol.png"
	...
	If File(aBmp2)
		...
		oPrint:Say  (0640,240,"Banco Rendimentos",oFont10 )
	Else
	...
	If File(aBmp2)
		...
		oPrint:Say  (2010,240,"Banco Rendimentos",oFont10 )			// [2]Nome do Banco
	Else
		oPrint:Say  (2014,100,aDadosBanco[2],oFont15n )		// [2]Nome do Banco                     1934
	EndIf

	oPrint:Say  (1988,569,"633",oFont22 )						// [1]Numero do Banco                       1912
	...
	If SE1->(dbSeek(xFilial('SE1') + aDadosTit[7] + aDadosTit[1], .F. ))
		...
		//SE1->E1_HIST   := "BOLETO BCO Rendimentos"
		MsUnlock()
	EndIf
