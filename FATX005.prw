...
Private oMsGet1			:= Nil
Private aVMsGet1		:= {"NOUSER","UI_ATEND","UI_EMISSAO","UI_INCLUI","UI_TIPO","UI_CODCLI","UI_LOJA","UI_NOMECLI","UI_CONTATO","UI_TELCTO","UI_SNOTA","UI_VISTECN","UI_NCONFOR","UI_NCONFCP","UI_DEVOLUC","UI_MOTDEVO"}
Private aAMsGet1		:= {"UI_ATEND","UI_EMISSAO","UI_INCLUI","UI_TIPO","UI_CODCLI","UI_LOJA","UI_NOMECLI","UI_CONTATO","UI_TELCTO","UI_SNOTA","UI_VISTECN","UI_NCONFOR","UI_NCONFCP","UI_DEVOLUC","UI_MOTDEVO"}
Private oGetD1			:= Nil                 
Private aTela1			:= {}
                                               
Private oMsGet2			:= Nil
Private aVMsGet2		:= {"NOUSER","UI_TERMINO","UI_ANALISE","UI_PROCEDE","UI_DEVOLUC","UI_PROBLEM","UI_INVMAQU","UI_INVMETO","UI_INVMEDI","UI_INVMATP","UI_INVMOBR","UI_INVMAMB","UI_CAUSA","UI_ACAO","UI_GRPINVE","UI_PLANO"}
Private aAMsGet2		:= {"UI_TERMINO","UI_ANALISE","UI_PROCEDE","UI_DEVOLUC","UI_PROBLEM","UI_INVMAQU","UI_INVMETO","UI_INVMEDI","UI_INVMATP","UI_INVMOBR","UI_INVMAMB","UI_CAUSA","UI_ACAO","UI_GRPINVE","UI_PLANO"}
Private aTela2			:= {}                                      
                                               
Private oMsGet3			:= Nil
Private aVMsGet3		:= {"NOUSER","UI_IMPLANT","UI_DETIMPL","UI_EFICAZ","UI_DETEFIC"}
Private aAMsGet3		:= {"UI_IMPLANT","UI_DETIMPL","UI_EFICAZ","UI_DETEFIC"}
Private aTela3			:= {}
...
If nOpc == 4
	...
	If (	Upper(AllTrim(cUserName)) == Upper(AllTrim(M->UI_INCLUI)) .And. ( M->UI_STATUS $ "4#5#7" ) )
		...
		If RTrim(Upper(M->UI_MOTDEVO)) $ "T#V#P"
			aAdd(aTitles, "2 - Análise e Plano de Ação")
			aAdd(aTitles, "3 - Eficácia")               
		EndIf 		
	...
	Else
		If Upper(RTrim(cUserName)) $ "CAROLINE.MONEA#LUANA.ARANEGA#ADMINISTRADOR" // 28/11/2016 Ajuste realizado para revisão dos registros para normas exigidas pela auditoria. LUANA.ARANEGA#
			//(cAlias)->UI_STATUS 	:= "6"  
			...
			aAMsGet1 := {"UI_NCONFOR","UI_NCONFCP","UI_CODCLI","UI_LOJA","UI_TIPO","UI_MOTDEVO","UI_SNOTA"}
			aAMsGet2 := {"UI_TERMINO","UI_ANALISE","UI_PROCEDE","UI_DEVOLUC","UI_PROBLEM","UI_INVMAQU","UI_INVMETO","UI_INVMEDI","UI_INVMATP","UI_INVMOBR","UI_INVMAMB","UI_CAUSA","UI_ACAO","UI_GRPINVE","UI_PLANO"}	
...
BEGIN SEQUENCE                         
	...
	If nOpc == 3
		...
		RecLock(cAlias, .T.)
			...
			(cAlias)->UI_STATUS		:= IIf( !(cAlias)->UI_MOTDEVO $ "T#V#P", "7", "1" ) //(cAlias)->UI_STATUS		:= IIf( (cAlias)->UI_MOTDEVO != "T", "7", "1" )
			(cAlias)->UI_DEVOLUC	:= IIf( !(cAlias)->UI_MOTDEVO $ "T#V#P", "2", "3" )
	...
	ElseIf nOpc == 4
		...
		RecLock(cAlias, .F.)                    
			...
			Else
				...
				(cAlias)->UI_STATUS		:= IIf( !(cAlias)->UI_MOTDEVO $ "T#V#P", "7", "1" )
				(cAlias)->UI_DEVOLUC	:= IIf( !(cAlias)->UI_MOTDEVO $ "T#V#P", "2", "3" )
				(cAlias)->UI_PROCEDE	:= IIf( (cAlias)->UI_PROCEDE != "T", "1", "2" )    													
	...
	ElseIf nOpc == 8
		...
		If (cAlias)->UI_MOTDEVO $ "T#V#P" .And. (cAlias)->UI_EFICAZ != "2"
		...
		RecLock(cAlias, .F.)
			...
			(cAlias)->UI_DEVOLUC	:= IIf((cAlias)->UI_DEVOLUC != "3", (cAlias)->UI_DEVOLUC, IIf((cAlias)->UI_MOTDEVO $ "T#V#P", "1", "2"))
		(cAlias)->( MsUnlock() )
...
If lOk .And. ( nOpc == 3 .Or. nOpc == 4 ) ;
		.And. ( ( Empty(M->UI_CODCLI) .Or. Empty(M->UI_LOJA) ) .And. !( M->UI_TIPO == "I" .And. M->UI_MOTDEVO == "G" ) ) //.And. ( Empty(M->UI_CODCLI) .Or. Empty(M->UI_LOJA) ) - Alterado 03/02/2016
