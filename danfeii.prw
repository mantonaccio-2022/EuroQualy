...
#DEFINE MAXITEMC   043//035					// Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXCODPRD  055 //050					// Máximo de caracteres do codigo de produtos/servicos conforme o tamanho do quadro "Cod. prod"
// - VERIFICAR SEMPRE QUANDO ATUALIZAR - 2022 - CUSTOM
/*/
...
If nTipo <> 1
	...
	PRIVATE oFont15N   := TFontEx():New(oDanfe,"Times New Roman",14,14,.T.,.T.,.F.)// custom - 2022
...
//Customizado Euroamerican - 2022 CUSTOM  - retira a mensagem de pedidos para deixar isolada em um array
...
For nX := 1 To Min(nLenMensagens, MAXMSG)
		If "|"  $ aMensagem[nX][1]  
			...
			cVar:=Substr(aMensagem[nX][1],nPos1,nPos2)
			aMensagem[nX][1]:=Substr(aMensagem[nx][1],1,nPos1-1)		
			aadd(aMensagem, { cVar , .F. } )  // cria um novo array de mensagem especifica pedido
		End
Next nX
...
//Next nX
For nX := 1 To Min(nLenMensagens, MAXMSG)
	//Alteração da Fonte  quando for imprimir  a Observação do Pedido de Venda - MAA 03/11/2021
	If "Pedido(s) de Venda: " $ aMensagem[nX][1]  
		nLin:=nlin+2 //3
		oDanfe:Say(nLin,002,aMensagem[nX][1],oFont15N:oFont)  //Customizado Euroamerican - Retirado
	Else
		oDanfe:Say(nLin,002,aMensagem[nX][1],oFont07:oFont)  //Customizado Euroamerican - Retirado
	End
	nLin:= nLin+10
Next nX
